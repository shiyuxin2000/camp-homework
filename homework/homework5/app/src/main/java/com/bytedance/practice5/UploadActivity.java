...
public class UploadActivity extends AppCompatActivity {
    ...
    private void submit() {
        ...
        MultipartBody.Part newto= MultipartBody.Part.createFormData("to",to);
        MultipartBody.Part newcontent= MultipartBody.Part.createFormData("content",content);
        Call<UploadResponse> response=api.submitMessage(Constants.STUDENT_ID,"",from,newto,newcontent,coverPart,Constants.token);
    ...
    private void submitMessageWithURLConnection(){
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                String urlStr = String.format(Constants.BASE_URL + "messages?student_id=%s", Constants.STUDENT_ID);
                try{
                    ...
                    Log.d(TAG,"where is the bug1?");

                    Log.d(TAG,"where is the bug2?");
